...
var displayMovieDetails = function (movies) {
  ...
  if (movies.poster_path) {
    ...
    $("#imgId").attr("src", moviePosterPath);
    $("#imgId").attr("style", "object-fit:cover;");
  } else {
    $("#imgId").attr("src", "./assets/images/no-poster.png");
  }
  ...
  titleEl.append(
    $("<p>")
      .css("display", "block")
      .append($("<strong>").text("Runtime : "))
      .append($("<p>").css("display", "inline-block").text(runtime))
  );
  if (userRating > 0) {
    titleEl.append(
      $("<p>")
        .css("display", "block")
        .append($("<strong>").text("User Rating : "))
        .append(
          $("<p>")
            .css("display", "inline-block")
            .text(userRating + "/ 10")
    ...
    titleEl.append(
      $("<p>")
        ...
        .append($("<strong>").text("User Rating : "))
        .append(
          $("<p>").css("display", "inline-block").text("No rating available")
        )
    );
  }
  titleEl.append(
    $("<p>")
      .css("display", "block")
      .append($("<strong>").text("Release Date : "))
      .append($("<p>").css("display", "inline-block").text(releaseDate))
  );
  titleEl.append(
    $("<p>")
      .css("display", "block")
      .append($("<strong>").text("Director : "))
      .append($("<p>").css("display", "inline-block").text(director))
  );
  titleEl.append(
    $("<p>")
      .css("display", "block")
    ...
    for (var i = 0; i < movies.credits.cast.length; i++) {
      if (movies.credits.cast[i].profile_path != null) {
        var castProfilePath =
          ...
          movies.credits.cast[i].profile_path;
      } else {
        var castProfilePath = "./assets/images/no-photo.jpeg";
      }
      var cardEl = $(
        '<div class="card small-12" style="height:320px; width:146px;"></div>'
      );
      var castImageEl = $('<img style="height:220px; object-fit:cover">').attr(
        "src",
        castProfilePath
      );
      var cardSectionEl = $(
...
function displayRatingsFromOmdb(ratings) {
  if (ratings.length !== 0) {
    ...
    var cardEl = $(
      '<div class="grid-x grid-padding-x align-middle text-center" style="height: auto; background-color:#201b1b;  border: 1px groove #333; padding: 10px; margin-top:15px;"></div>'
    );
    for (var i = 0; i < ratings.length; i++) {
      ...
      if (ratingValue) {
        ...
        if (cellCreated === false) {
          var cardSectionEl = $(
            '<div class="cell small-12 medium-3 align-self-left" style="padding:0px;"></div>'
          );
          cellCreated = true;
        } else {
          var cardSectionEl = $(
            '<div class="cell small-12 medium-3 align-self-center" style="padding:0px;"></div>'
          );
        }
...
function displayReviews(reviews) {
  var sectionReviewEl = $(
    '<div style="background-color:#1B1616; padding :15px; margin-bottom:10px;"></div>'
  );
  ...
  if (reviews.length !== 0) {
    ...
    sectionReviewEl.append(
      $("<h6>").append($("<strong>").text("A review written by " + author))
    );
    sectionReviewEl.append(
      $("<p>").text("Written by " + author + " on " + reviewDate)
    );
    sectionReviewEl.append(
      $("<p>")
        .css("display", "block")
    ...
    //start voice -button
    var buttonPlayEl = $(
      '<button id="play-review" class="button" type="button" style="display:inline;"></button>'
    );
    ...
    $("#read-more-reviews").on("click", function () {
      ...
      for (var i = 0; i < reviews.length; i++) {
        var sectionReviewEl = $(
          '<div style="background-color: white; padding :15px"></div>'
        );
    ...
    titleEl.append($("<h4>").append($("<strong>").text("Reviews")));
    titleEl.append(
      $('<h5 style="width:600px">').text("No reviews").css("display", "block")
    );
  }
}
