...
var displayMovieDetails = function (movies) {
  ...
  for (var i = 0; i < movies.credits.cast.length; i++) {
    if (movies.credits.cast[i].profile_path != null) {
      ...
      var cardEl = $(
        '<div class="card small-12" style="height:320px; width:170px ; background-color:rgb(227, 212, 212)"></div>'
      );
      var castImageEl = $('<img style="height:200px; width:200px">').attr(
        "src",
        castProfilePath
      );
      var cardSectionEl = $(
...
function displayRatingsFromOmdb(ratings) {
  if (ratings.length !== 0) {
    ...
    var ratingValue;
    var cardEl = $(
      '<div class="grid-x grid-padding-x align-center-middle text-center" style="height: 150px;"></div>'
    );
...
function displayReviews(reviews) {
  if (reviews.length !== 0) {
    ...
    sectionReviewEl.append(
      $("<p>").text("Written by " + author + " on " + writtenDate)
    );
    sectionReviewEl.append(
      $("<p>")
        .css("display", "block")
    ...
    $("#read-more-reviews").on("click", function () {
      ...
      for (var i = 0; i < reviews.length; i++) {
        ...
        var writtenDate = dayjs(reviews[i].created_at.substring(0, 7)).format(
          "DD-MM-YY"
        );
        ...
        var buttonPlayEl = $(
          '<button id="play-review" class="button" type="button" style="display:inline-block;"></button>'
        );
        //  Screen readers will see "Play"
        var spanScreenReaderEl = $('<span class="show-for-sr">Play</span>');
        // Visual users will see the icon , but not the "Play" text
