...
var getMovieById = function (movieId) {
  ...
  if (movies.credits.cast.length > 0) {
    for (var i = 0; i < movies.credits.cast.length; i++) {
      ...
      var cardEl = $(
        '<div class="card small-12" style="height:320px; width:170px ; background-color:white"></div>'
      );
      var castImageEl = $('<img style="height:220px; ">').attr(
        "src",
        castProfilePath
      );
      var cardSectionEl = $(
...
var getMovieByImdbId = function (imdbId) {
  ...
  function displayRatingsFromOmdb(ratings) {
    if (ratings.length !== 0) {
      ...
      var cardEl = $(
        '<div class="grid-x grid-padding-x align-middle text-center" style="height: 150px;"></div>'
      );
...
function displayReviews(reviews) {
  var sectionReviewEl = $(
    '<div style="background-color:#1B1616; padding :15px"></div>'
  );
  ...
  if (reviews.length !== 0) {
    ...
    var sectionReviewEl = $(
      '<a id="read-more-reviews" style="margin-left:0px">Read all Reviews</a>'
    );
    titleEl.append(sectionReviewEl);
